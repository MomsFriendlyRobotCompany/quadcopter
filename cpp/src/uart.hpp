...
class Serial {
  ...
  inline
  bool is_enabled() { return uart_is_enabled(uart); }

  inline
  void flush() { buffer->clear(); }

  inline
  uint set_baud(uint baud) { return uart_set_baudrate(uart, baud); }

  inline
  size_t available() { return buffer->size(); }
  ...
  inline
  uint8_t read() { return buffer->pop(); }
